...
void ASimModeWorldMultiRotor::setupVehiclesAndCamera(std::vector<VehiclePtr>& vehicles)
{
	...
	//find all vehicle pawns
	{
		...
		for (AActor* pawn : pawns)
		{
			...
			vehicle_pawn->initializeForBeginPlay(getSettings().additional_camera_settings);///////////////////////////////////add camera
...
std::string ASimModeWorldMultiRotor::getLogString()
{
	...
	SimpleFlightQuadX& mrp = static_cast<SimpleFlightQuadX&>(*vehicle_params_[0]); // add by lmy, testing
	SensorCollection *s = &mrp.getSensors();// add by lmy, testing
	auto imub = static_cast<const ImuBase*>(s->getByType(SensorBase::SensorType::Imu));//add by lmy,testing
	msr::airlib::ImuBase::Output imu_out = imub->getOutput();//add by lmy,testing
	auto gps = static_cast<const GpsBase*>(s->getByType(SensorBase::SensorType::Gps));
	msr::airlib::GpsBase::Output gps_out = gps->getOutput();
	auto magnetometer = static_cast<const MagnetometerBase*>(s->getByType(SensorBase::SensorType::Magnetometer));
	msr::airlib::MagnetometerBase::Output magnetometer_out = magnetometer->getOutput();
	...
	std::string line;
	line.append("Timestamp:").append(std::to_string(timestamp_millis)).append("\t")
		.append("Position:").append(std::to_string(kinematics->pose.position.x())).append("\t")
		.append(std::to_string(kinematics->pose.position.y())).append("\t")
		.append(std::to_string(kinematics->pose.position.z())).append("\t")
		.append("orientation(wxyz):").append(std::to_string(kinematics->pose.orientation.w())).append("\t")
		...
		.append("IMU_angular_velocity:").append(std::to_string(imu_out.angular_velocity.x())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.angular_velocity.y())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.angular_velocity.z())).append("\t")//add by lmy,testing
		.append("IMU_linear_acceleration:").append(std::to_string(imu_out.linear_acceleration.x())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.linear_acceleration.y())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.linear_acceleration.z())).append("\t")//add by lmy,testing
		.append("IMU_orientation:").append(std::to_string(imu_out.orientation.w())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.orientation.x())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.orientation.y())).append("\t")//add by lmy,testing
		.append(std::to_string(imu_out.orientation.z())).append("\t")//add by lmy,testing
		.append("gps_latitude:").append(std::to_string(gps_out.gnss.geo_point.latitude)).append("\t")//gps
		.append("gps_longitude:").append(std::to_string(gps_out.gnss.geo_point.longitude)).append("\t")
		.append("gps_altitude:").append(std::to_string(gps_out.gnss.geo_point.altitude)).append("\t")
		.append("magnetic field in body frame:").append(std::to_string(magnetometer_out.magnetic_field_body.x())).append("\t")//magnetic field body  leave out covariance matrix
		.append(std::to_string(magnetometer_out.magnetic_field_body.y())).append("\t")
		.append(std::to_string(magnetometer_out.magnetic_field_body.z())).append("\t")
		;
	return line;
