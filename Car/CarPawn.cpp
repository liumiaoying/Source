...
ACarPawn::ACarPawn()
{
	...
	//为车辆添加camera，实际读取camera信息会出错，可能是内存相关
	//InternalCameraBase6 = CreateDefaultSubobject<USceneComponent>(TEXT("InternalCameraBase6"));
	//InternalCameraBase6->SetRelativeLocation(FVector(35.0f, -10, 50.0f)); //left
	//InternalCameraBase6->SetupAttachment(GetMesh());
	//InternalCameraBase7 = CreateDefaultSubobject<USceneComponent>(TEXT("InternalCameraBase7"));
	//InternalCameraBase7->SetRelativeLocation(FVector(35.0f, 10, 50.0f)); //right
	//InternalCameraBase7->SetupAttachment(GetMesh());
...
void ACarPawn::initializeForBeginPlay(bool enable_rpc, const std::string& api_server_address, bool engine_sound)
{
	...
	/*额外相机
	InternalCamera6 = this->GetWorld()->SpawnActor<APIPCamera>(pip_camera_class_, camera_transform, camera_spawn_params);
	InternalCamera6->AttachToComponent(InternalCameraBase6, FAttachmentTransformRules::KeepRelativeTransform);
	InternalCamera7 = this->GetWorld()->SpawnActor<APIPCamera>(pip_camera_class_, camera_transform, camera_spawn_params);
	InternalCamera7->AttachToComponent(InternalCameraBase7, FAttachmentTransformRules::KeepRelativeTransform);*/
